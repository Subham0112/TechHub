...
const ProductCard: React.FC<{
  ...
  isAdmin: boolean;
}> = ({ product, handleCartClick, isAdmin }) => {
  const navigate = useNavigate()
  const productHref = `/products/${product.slug}-${product._id}`

  return (
    <div className="group relative bg-[#121A2E] rounded-xl overflow-hidden border border-[#232F49] hover:border-[#5B8DEF]/60 transition-all duration-300 hover:-translate-y-1.5 flex-shrink-0 w-[240px] flex flex-col">
      ...
      <div className="relative h-[180px] overflow-hidden bg-[#0A0E1A] flex-shrink-0">
        ...
        {!isAdmin && (
          <div className="absolute inset-0 bg-gradient-to-t from-[#0A0E1A] via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center gap-2 pb-3">
            <button
              onClick={() => navigate(productHref)}
              className="px-3.5 py-1.5 bg-[#182238] border border-[#5B8DEF]/40 text-[#5B8DEF] rounded-full font-mono font-semibold cursor-pointer text-[11px] uppercase tracking-wide hover:bg-[#5B8DEF]/10 transition-all flex items-center gap-1.5"
            >
            ...
            <button
              ...
              className="px-3.5 py-1.5 bg-[#FFB238] text-[#0A0E1A] rounded-full font-mono font-semibold cursor-pointer text-[11px] uppercase tracking-wide hover:bg-[#ffc158] transition-all flex items-center gap-1.5 disabled:opacity-40 disabled:cursor-not-allowed"
            >
      ...
      <div className="p-3.5 flex flex-col flex-1">
        ...
        <h3
          onClick={() => navigate(productHref)}
          className="text-[#EDF1F7] font-display font-medium text-sm mt-1 mb-3 line-clamp-2 min-h-[2.5rem] cursor-pointer hover:text-[#5B8DEF] transition-colors"
        >
          {product.name}
        </h3>

        <div className="flex items-center justify-between pt-2 border-t border-[#232F49] mt-auto">
